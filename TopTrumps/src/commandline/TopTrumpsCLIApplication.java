...
public class TopTrumpsCLIApplication {
	...
	public static void main(String[] args) {
		...
		TopTrumpsCLIApplication TopTrumps = new TopTrumpsCLIApplication("StarCitizenDeck.txt", 40, "Human");// The class																								// played
		...
		while (!TopTrumps.userWantsToQuit) {
			...
			/*if (args.length > 1) { // It is verified if arguments were passed to the execution of the program to
									// know what the game mode will be
				if (!args[1].equals("-b")) { // If the argument is different from -b then the mode chosen is that a
												// human play against the 4 robots.
					...
					String[] CompPlayerNames = { "BotOne", "BotTwo", "BotThree", "BotFour" };// We create an arrangement
																								// of the names of our
																								// robots
					for (int i = 1; i < TopTrumps.numPlayers; i++) { // Here we create one by one the additional players
																		// apart from the player one that would be the
																		// human
						TopTrumps.NewPlayers[i] = new Player(CompPlayerNames[i - 1]);
					}
					TopTrumps.Players = TopTrumps.NewPlayers;

					Random rand = new Random();// We create the random object of java that will help us so that the
												// system randomly decides which player to start the game
					int decidingPlayerIndex = rand.nextInt(TopTrumps.Players.length);// We use its nextInt method and we
																						// pass the amount of players
																						// that will play to give us the
																						// index of the player who
																						// started the game.
					...
					for (int i = 0; i < TopTrumps.no_cards; i++) {
						...
						playerC.giveCard(TopTrumps.deck.getDeck().get(i), p);
					}
					String UserCardInfo;
					...
					if (user.getHand().length == 0) {
						...
						UserCardInfo = CardDescription + CardAttribute1 + CardAttribute2 + CardAttribute3
								+ CardAttribute4 + CardAttribute5;
					}
					...
					for (Player P : TopTrumps.Players) {
						...
						String attrCardsNameString = String.format(
								"%20.20s %15.15s %15.15s " + "%15.15s %15.15s %15.15s", "", attrCard1, attrCard2,
								attrCard3, attrCard4, attrCard5);
						...
						for (Card hand : P.getHand()) {
							...
							String attValString = String.format("%20.20s %15.15s %15.15s " + "%15.15s %15.15s %15.15s",
									nameValue, attribute1Val, attribute2Val, attribute3Val, attribute4Val,
									attribute5Val);
			...
			/*
			 * } }else{ System.out.
			 * println("You must enter a numerical value between the opciones given");
	...
	private void checkIfGameOver(int type) {
		if (type == 1) {
			...
			for (int i = 0; i < this.numPlayers; i++) {
				...
				/*
				 * if (user.getHand().length ==
				 * this.Round.getDeck().getDeck().size()-this.Round.getPile().getCards().length
	...
	private void playRound(int trumpIndex, int type) {
		...
		this.prevRoundString = roundV.getRoundString(type,this.Round);
		...
		if (type == 1) {
			...
			System.out
					.println("Cards left in hand: " + Players[0].getHand().length + "\nCurrent card: " + UserCardInfo);
